        ...
        joy_sub_ = this->create_subscription<sensor_msgs::msg::Joy>(
            ...
            [this](const sensor_msgs::msg::Joy::SharedPtr msg) {
                ...
                if (smoothed_speed1_ >= 0) {
                    send_command(0x80, 0x01, static_cast<uint8_t>(smoothed_speed1_)); //moter1 nomal rotation
                    send_command(0x80, 0x04, static_cast<uint8_t>(smoothed_speed1_)); //moter2 nomal rotation
                } else{
                    send_command(0x80, 0x00, static_cast<uint8_t>(-smoothed_speed1_)); //moter1 reversal rotation
                    send_command(0x80, 0x05, static_cast<uint8_t>(-smoothed_speed1_)); //moter2 reversal rotation

                }
            

                /*control moter2*/
                float raw2 = msg->axes[0];
                ...
                if (smoothed_speed2_ >= 0) {
                    send_command(0x80, 0x01, static_cast<uint8_t>(smoothed_speed1_)); 
                    
                } else{
                    send_command(0x80, 0x04, static_cast<uint8_t>(-smoothed_speed2_)); //moter2 reversal rotation
                }
