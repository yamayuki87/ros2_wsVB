    ...
    set_position_subscriber_ =
      this->create_subscription<sensor_msgs::msg::Joy>(
        ...
        [this](const sensor_msgs::msg::Joy::SharedPtr msg)
        {
          ...
          int target_position = static_cast<int>((raw + 1.0) / 2.0 * 4096); 
          ...
          if (dxl_comm_result != COMM_SUCCESS) {
            ...
            RCLCPP_INFO(this->get_logger(),
              "Set [ID: %d] [Goal Position: %d]", 1, target_position);
          }
        });

    // --- Service for reading position ---
    get_position_server_ =
      this->create_service<dynamixel_sdk_custom_interfaces::srv::GetPosition>(
        "get_position",
        [this](
...
void setupDynamixel(uint8_t dxl_id)
{
  ...
  if (dxl_comm_result != COMM_SUCCESS) {
    RCLCPP_ERROR(rclcpp::get_logger("dynamixel_setup"), "Failed to set Position Control Mode.");
  } else {
    RCLCPP_INFO(rclcpp::get_logger("dynamixel_setup"), "Succeeded to set Position Control Mode.");
  }
...
int main(int argc, char * argv[])
{
  ...
  setupDynamixel(1);//setup for ID=1
